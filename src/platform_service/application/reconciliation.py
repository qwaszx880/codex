"""Provider-neutral command processing for management-cluster executors."""
...
class CommandProcessor:
    """Apply one exact desired revision with idempotency and per-cluster locking."""
    ...
    def process(self, message: dict) -> None:
        """Process an at-least-once command or classify it as a safe no-op."""
        ...
        # Event IDs are the consumer-side idempotency key across redeliveries.
        ...
        # Never let a delayed command roll desired state back to an older revision.
        if target != cluster.desired_revision:
            raise StaleOperation(f"revision {target} is not desired {cluster.desired_revision}")
        # The lease serializes only this cluster; other clusters remain concurrent.
