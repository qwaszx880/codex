"""HTTP transport for cluster lifecycle and observed-state queries.

Routes only translate HTTP concerns and delegate mutations to ``ClusterService``.
They deliberately do not compile resources or publish queue messages directly.
"""
...
def resolve_request_id(value: str | None = Header(None, alias="X-Request-ID")) -> str:
    """Preserve a caller correlation ID or create one at the HTTP boundary."""

    return value or uuid4().hex


def translate_domain_errors(call: Callable[[], Result]) -> Result:
    """Map expected application errors without hiding unexpected failures."""
...
def authorized_cluster(
    ...
    """Load a cluster only after checking authorization in its owning project."""
...
def upgrade(
    ...
    data = deepcopy(rev.spec)
    data["kubernetes"]["version"] = body.version
    _, op = translate_domain_errors(
        lambda: ClusterService(session).revise(
            cluster_id=cluster_id,
            spec=ClusterSpec.model_validate(data),
            kind="UPGRADE",
            reason=f"upgrade to {body.version}",
            actor_id=identity.principal_id,
            permissions=project_permissions(session, identity.principal_id, cluster.project_id),
            request_id=request_id,
        )
