...
For a field-level database reference, maintainable Mermaid entity-relationship diagram,
and guidance for polling asynchronous operations from a frontend, see the
...
- `ClusterUpdate` and worker-node-type models describe revision-producing changes;
- `ClusterView` exposes revision pointers without provider secrets;
- `OperationView` exposes asynchronous user intent;
- administration models expose principals, projects, node profiles, and safe provider
  metadata without returning credential references;
...
Organization administration routes provision external principal mappings and projects;
project administration routes manage provider references and reusable node profiles.
...
- `replace_spec()` and the worker-node-type helpers append general desired-state updates;
- `delete()` appends a deletion revision and emits the same durable command contract;
...
### `src/platform_service/application/admin_service.py`

Owns synchronous, audited organization/project administration, external principal
provisioning, provider-reference management, and reusable node-profile management.
Principal creation stores an `(issuer, external_subject)` mapping and organization role,
not an identity-provider account or password. Provider API responses deliberately omit
...
7. compiles and applies provider resources, or calls the adapter's deletion boundary;
...
Covers audited principal/project/provider/profile administration and confirms that
provider responses do not expose the stored secret reference.
...
Verifies upgrades, worker-node-type changes, durable event identity, revisioned deletion,
and dispatch through the management adapter's deletion boundary.
...
missing nodes, audit-read, and metrics APIs, a fake rather than Kubernetes-backed
