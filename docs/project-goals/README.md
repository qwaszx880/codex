...
59. **Profile changes.** Worker/control-plane profile changes create revisions and
    operations.
60. **Autoscaling.** Compile global or per-worker autoscaling intent to Cluster Autoscaler
    min/max annotations while leaving replica ownership to the autoscaler.
...
| Revisions (13–14, 57–59) | **Partial** | Create, full-spec update, worker-node-type changes, scale, upgrade, and delete append specs and track desired/applied/observed revisions; stale commands are rejected. | Add stronger concurrent mutation tests and provider-aware validation. |
| Compiler/provider abstraction (15–19, 90) | **Partial** | `ClusterCompiler`, `ManagementClusterAdapter`, and `SecretStore` ports exist; control-plane and worker machine templates are fully linked, and executor-resolved node profiles populate CAPO flavor/image/storage and placement fields. Provider references supply identity/network configuration without exposing credential values. | Implement a real Kubernetes/CAPO adapter and secret-store adapter, validate profiles against live provider capabilities, and add a second-provider contract test. |
...
| Observed state and health (41–48) | **Partial** | Fake observer stores the required simulated kinds, latest raw status, an initial condition-history row, normalized health, and independent operation outcomes. | Append and deduplicate later condition transitions, model generation/resource-version changes, implement real Kubernetes watches and resync/restart behavior, and add CAPO-specific normalization tests. |
...
| Lifecycle and read APIs (56–63) | **Partial** | Create/list/get/update/delete, worker-node-type changes, scale, upgrade, revisions, operations, health, conditions, and resources exist. Delete is revisioned and the fake plane retains a tombstone/history. Tenant administration covers principal mappings, projects, provider references, and node profiles. | Deploy/configure Cluster Autoscaler and addon source resources in a real management cluster; add nodes, audit reads, richer operation responses, and production deletion observation. |
...
| Local development (77–84) | **Mostly implemented** | Compose supplies PostgreSQL/Adminer, RabbitMQ UI, Keycloak, API, publisher, scalable executor, fake observer, and Flower using the shared desired-state model. The fake observer covers create, update, scale, upgrade, deletion, and whole-poll success/failure snapshots. | Add multi-step condition-transition simulation and automate every documented failure scenario as integration tests. |
