...
| Lifecycle and read APIs (56–63) | **Partial** | Create/list/get, scale, upgrade, revisions, operations, health, conditions, and resources exist. Autoscaling annotations, machine health checks, and ConfigMap/Secret-backed ClusterResourceSets compile from desired state. | Deploy/configure Cluster Autoscaler and addon source resources in a real management cluster; add PATCH, DELETE, nodes, full asynchronous deletion/tombstones, and missing response schemas. |
...
| Local development (77–84) | **Mostly implemented** | Compose supplies PostgreSQL/Adminer, RabbitMQ UI, Keycloak, API, publisher, scalable executor, fake observer, and Flower using the shared desired-state model. The fake observer covers create, scale, upgrade, and whole-poll success/failure snapshots. | Add deletion and multi-step condition-transition simulation, and automate every documented failure scenario as integration tests. |
